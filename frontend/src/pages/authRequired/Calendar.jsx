...
function CalendarPage() {
    ...
    const [eventModalVisibility, setEventModalVisibility] = React.useState(false);
	...
	if (!event) {
	    throw new Error(`Couldn't find event: ${info.event.title} @ ${info.event.start}`);
	};

	setEventName(event.dutyName);
	setEventDate(new Date(event.day).toLocaleDateString("en-us", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }));
